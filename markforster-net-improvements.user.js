...
function createMenu() {
  ...
  menuBox.innerHTML =
    '<div class="iw-s1"><div class="iw-s2">' +
...
function modifyLinks() {
  var topicLinks = document.evaluate(
    "//a[contains(@href,'/forum/post/')]",
    document,
    null,
...
function highlightNewTopics() {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    for (var j = 0; j < cells.length; j++) {
      if (count == 1) {
        if (
          cells[j].getAttribute("class") == "replycount-cell" &&
        ...
        if (cells[j].getAttribute("class") == "updated-cell") {
          ...
          if (!tempReplies && lastChecked == cells[j].childNodes[1].innerHTML) {
            ...
            if (debug && !tempReplies)
              alert(lastChecked + "!= " + cells[j].childNodes[1].innerHTML);
            changes = 1;
...
function highlightPosts() {
  var divs = document
    ...
    .getElementsByClassName("signature");
  var first = 1;
  var date, post;
  for (var i = 0; i < divs.length; i++) {
    ...
    if (
      isHighlightNewPosts == "1" &&
      previousChecked != "" &&
      isNewerTopic(previousChecked, date)
    ) {
      ...
      if (
        ...
        post
          ...
          .setAttribute("style", "background-color:#AFA;");
      } else if (
        isHighlightNorman == "1" &&
        ...
        post
          .getElementsByClassName("comment")[0]
          .setAttribute("style", "background-color:#FAA;");
      } else {
        post
          .getElementsByClassName("comment")[0]
          .setAttribute("style", "background-color:#FFA;");
      }
      if (first) {
        first = 0;
    ...
    if (isNormanInBlack == "1" && divs[i].innerHTML.indexOf("Norman U") != -1) {
      post
        .getElementsByClassName("comment")[0]
        .setAttribute("style", "background-color:#000;color:#000;");
    }
    if (
      isShowOnlyNorman == "1" &&
      divs[i].innerHTML.indexOf("Norman U") == -1
